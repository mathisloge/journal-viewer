...
class JournalLruEntryChunkCacheNew
{
  public:
    // Konstruktor: capacity = maximale Anzahl an Chunks, chunkSize = Anzahl Einträge pro Chunk.
    explicit JournalLruEntryChunkCacheNew(size_t capacity, size_t chunk_size)
        : capacity_(capacity)
    ...
    [[nodiscard]] const JournalEntry &get_entry(std::uint64_t global_index, auto &&chunk_loader)
    {
        ...
        chunk_loader(chunk_start);
        return kDefaultEntry;
    }

    void emplace_chunk(std::uint64_t chunk_start, std::vector<JournalEntry> &&new_chunk)
    {
        if (cache_map_.contains(chunk_start))
        {
            return;
        }
        fmt::println("loaded chunk for start {}", chunk_start);
